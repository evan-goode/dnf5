...
constexpr const char * PLUGIN_NAME{"rebuild"};
...
constexpr const char * attrs_value[]{"Evan Goode", "egoode@redhat.com", "Layer packages on bootc systems"};

class RebuildCmdPlugin : public IPlugin {
public:
...
std::vector<std::unique_ptr<Command>> RebuildCmdPlugin::create_commands() {
    std::vector<std::unique_ptr<Command>> commands;
    commands.push_back(std::make_unique<RebuildCommand>(get_context()));
    return commands;
}
...
IPlugin * dnf5_plugin_new_instance([[maybe_unused]] ApplicationVersion application_version, Context & context) try {
    return new RebuildCmdPlugin(context);
} catch (...) {
