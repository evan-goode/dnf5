...
void RebuildRemoveCommand::run() {
    ...
    for (const auto & spec : pkg_specs) {
        auto it = std::find(installs.begin(), installs.end(), spec);
        if (it == installs.end()) {
            ctx.print_info(libdnf5::utils::sformat(
                _("Package spec \"{}\" is not in the input file, skipping. dnf-rebuild does not yet have the "
                  "capability to remove packages from the base system."),
                spec));
            continue;
        }
        installs.erase(it);
